...
// Функция создания матрицы графа
...
// Функция создания матрицы орграфа 
...
// Функция нахождения радиуса и диаметра 
int* findRadiusAndDiameter(int** G, int size, int& radius, int& diameter) {
	...
	for (int i = 0; i < size; i++) {
		...
		for (int j = 0; j < size; j++) {
			if (distances[j] > maxEccentricity && distances[j] != 0) {
...
// Основная с вызовом через командную строку 
int main(int argc, char* argv[])
{
	...
	if (argc != 3) {
		cerr << "Введите: " << argv[0] << " в командную строку(вызвать \"Win\" + \"R\") <взвешенность (0 или 1)> <ореинтированность (0 или 1)>" << endl;
		return 1;
	}
